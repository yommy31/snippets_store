...
const SnippetForm = dynamic(() => import('@/components/snippets/snippet-form').then(mod => mod.SnippetForm), {
  ssr: false
});
...
export function SnippetDetail() {
  ...
  // Handle create new snippet button click
  const handleCreateNewSnippet = () => {
    setSelectedSnippetId(undefined);
    setEditorMode('create');
  };
  
  // Reset editor mode when selected snippet changes
  useEffect(() => {
    if (selectedSnippetId) {
      setEditorMode('view');
    }
  }, [selectedSnippetId, setEditorMode]);
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="text-center">
          ...
          {/* Only show create button if not in recycle bin */}
          {currentView !== 'recycleBin' && (
            <Button 
              className="mt-4"
